...
const RestaurantDishes = () => {
  ...
  // Tracks whether the next fetch triggered by fetchTrigger must bypass browser HTTP cache
  const shouldBypassCache = useRef(false);

  // Immediately replace a single dish in state (optimistic update after edit)
  const updateSingleDish = useCallback((updatedDish) => {
    setDishes(prev =>
  ...
  const DishList = React.memo(({ dishes, categoryName, subCategoryName, restaurantId }) => {
    return dishes.map((dish) => (
      <DishCard
        ...
        onDishUpdate={forceRefresh}
        onDishEdit={updateSingleDish}
      />
    ));
  });
