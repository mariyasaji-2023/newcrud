...
export const createRestaurant = async (req, res) => {
  try {
    ...
    invalidateCache('totalRestaurants');
...
export const deleteRestaurant = async (req, res) => {
  ...
  try {
    ...
    invalidateCache('totalRestaurants', 'totalDishes');
...
export const createDish = async (req, res) => {
  ...
  try {
    ...
    await restaurant.save();
    invalidateCache('totalDishes');

    res.status(201).json({
      message: 'Dish created successfully',
      dish: target.dishes[target.dishes.length - 1],
    });
  } catch (error) {
...
export const totalRestaurants = async (req, res) => {
  try {
    const CACHE_KEY = 'totalRestaurants';
    let count = getCache(CACHE_KEY);
    if (count === null) {
      count = await Restaurant.countDocuments();
      setCache(CACHE_KEY, count, 2 * 60 * 1000);
    }
...
export const totalDishes = async (req, res) => {
  try {
    const CACHE_KEY = 'totalDishes';
    let total = getCache(CACHE_KEY);
    if (total === null) {
      const result = await Restaurant.aggregate([
            ...
            dishCount: {
              $reduce: {
                ...
                in: {
                  $add: [
                    '$$value',
                    { $size: { $ifNull: ['$$this.dishes', []] } },
                    {
                      $reduce: {
                        input: { $ifNull: ['$$this.subCategories', []] },
                        initialValue: 0,
                        in: {
                          $add: [
                            '$$value',
        ...
        { $group: { _id: null, total: { $sum: '$dishCount' } } },
      ]);
      total = result[0]?.total ?? 0;
      setCache(CACHE_KEY, total, 2 * 60 * 1000);
    }
